...
public class BTreeLatex {
    ...
    private StringBuilder commandsBuffer;
...
//        manualTest();
    ...
    private boolean startTests(int i) {
        ...
        if (!result) {
            try {
                ...
                writer.append(commandsBuffer.toString());
                writer.close();
            } catch (Exception e) {
                e.printStackTrace();
            }
    ...
    private boolean testDelete(int bound) {
        ...
        try {
            ...
            for (int i = 0; i < deletions.length - 1 & result; i++) {
                ...
                commandsBuffer.append("tree.delete(").append(deletions[i]).append(");\n");
            ...
            if (result & deletions.length > 0) {
                clearBuffer();
                commandsBuffer.append("tree.delete(").append(deletions.length - 1).append(");\n");
    ...
    public boolean testSearch(int bound) {
        ...
        try {
            ...
            for (int i = 0; i < bound & result; i++) {
                clearBuffer();
                commandsBuffer.append("tree.search(").append(searchs[i]).append("); \n");
    ...
    public boolean initSimpleTree(int bound) {
        boolean result = true;
        try {
            commandsBuffer.append("BTree tree = new BTree(").append(tree.getT()).append("); \n BTreeLatex manualTesto=new BTreeLatex(tree,\"manualTest\");\n");
            insertions = arrScrambler(bound);
            for (int i = 0; i < bound & result; i++) {
                clearBuffer();
                addTreeState("before inserting " + insertions[i]);
                commandsBuffer.append("tree.insert(new Block(").append(insertions[i]).append(",null));\n");
